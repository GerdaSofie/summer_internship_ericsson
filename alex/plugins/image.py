...
class Image(object):
    ...
    def show_images(self, images, width, height):
        ...
        image_count_side = self.sizefactor(len(images))
        image_width = width / image_count_side
        image_height = height / image_count_side

        self.display = pygame.display.set_mode(surface_size, 0)
        self.snapshot = pygame.surface.Surface(surface_size, 0, self.display)
        
        for idy in range(image_count_side):
            for idx in range(image_count_side):
                image_index = idy*image_count_side + idx
                if image_index >= len(images):
                    break
                image = images[image_index]
                img = pygame.image.load(StringIO(image))
                img = pygame.transform.scale(img, (image_width, image_height))
                self.display.blit(img, (idx*image_width, idy*image_height))
        pygame.display.flip()
